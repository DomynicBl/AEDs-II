...
public class Insertion_Sort {
    ...
    public static void insercao(Personagem[] registros) {
        try{
            FileWriter writer = new FileWriter("822331_insercao.txt",true);
            ...
            writer.write("822331"+ "\t" + duration + "\t" + comp + "\t"+ mov + "\t");
            writer.close();
    }catch(IOException e){
        e.printStackTrace();
    }
    ...
    public static void selecao(Personagem[] registros){
        try{
        FileWriter writer = new FileWriter("822331_selecao.txt",true);
        float startTime = System.nanoTime();
        int n = registros.length;
        int comp = 0;
        for(int i = 0; i<n-1;i++){
